...
(function($) {
    ...
    function lsubmitForm() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.verified) {
                    lformSuccess(data.msg);
                    setTimeout(()=>{
                        window.location.href = data.redirect_url
                    }, 300)
                } else {
                    lformError();
                    lsubmitMSG(false, data.msg);
                }
            }
        });
	}

    function lformSuccess(msg) {
        //$("#logInForm")[0].reset();
        lsubmitMSG(true, msg);
        //$("input").removeClass('notEmpty'); // resets the field label after submission
    }
    ...
    /* Newsletter Form */
    $("#subscirbeForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            nformError();
    ...
    function nformSuccess() {
        ...
        nsubmitMSG(true, "You will be redirected to login page!");
        $("input").removeClass('notEmpty'); // resets the field label after submission
    }

    function nformError() {
        $("#subscirbeForm").removeClass().addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
            $(this).removeClass();
        });
	}
